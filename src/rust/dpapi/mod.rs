...
pub mod dpapi {
    ...
    pub fn dpapi_protect(
        ...
        let protected = protect(data, optional_entropy, machine_scope).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("DPAPI error: {:?}", e))
        })?;
    ...
    pub fn dpapi_unprotect(
        ...
        let unprotected = unprotect(data, optional_entropy).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("DPAPI error: {:?}", e))
        })?;
